...
from utils import load_file, TASK_INST, PROMPT_DICT, save_file_jsonl, \
    process_arc_instruction, postprocess_answers_closed
...
def call_model_instructgpt(prompt, model, max_tokens=50):
    try:
        results = completions_instructgpt_backoff(model=model, prompt=prompt,
                                                  temperature=0.0,
                                                  max_tokens=max_tokens,
                                                  logprobs=5, top_p=1,
...
def main():
    ...
    isOpenAI = True if args.model_name in ["text-davinci-003",
                                           "gpt-3.5-turbo-0301",
                                           "gpt-3.5-turbo"] else False
    ...
    if args.mode == "retrieval":
        if args.retrieval_file is not None:
            ...
            for id, item in enumerate(input_data):
                retrieval_result = id2retrieval[
                    id if "id" not in item else item["id"]]
                evidences = ["[{}] ".format(
                    i + 1) + ctx["title"] + "\n" + ctx["text"] for i, ctx in
                             enumerate(retrieval_result)]
                item["paragraph"] = "\n".join(evidences)
        else:
            for id, item in enumerate(input_data):
            ...
            for input_instance in processed_batch:
                if args.model_name == "text-davinci-003":
                    pred = call_model_instructgpt(
                        input_instance, model=args.model_name,
