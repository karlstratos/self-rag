...
def run_step_generation_batch(model, prompt, paragraphs, max_new_tokens,
                              rel_tokens=None, grd_tokens=None, ret_tokens=None,
                              ut_tokens=None,
                              threshold=None, w_rel=1.0, w_sup=1.0, w_use=0.5,
                              use_seqscore=False):
    if paragraphs is not None:
        aug_prompts = [
            prompt + "[Retrieval]" + "<paragraph>{}</paragraph>".format(
                paragraph["title"] + "\n" + paragraph["text"]) for paragraph in
            paragraphs]
    else:
        aug_prompts = [prompt]
    ...
    for p_idx, pred in enumerate(preds):
        ...
        if len(ut_score_dict[p_idx]) == 5:
            ...
            utility_score = np.sum([ut_scores[i] * (ut_score_dict[p_idx][
                                                        "[Utility:{}]".format(
                                                            i + 1)] / ut_sum)
                                    if "[Utility:{}]".format(i + 1) in
                                       ut_score_dict[p_idx] else 0.0 for i in
                                    range(0, 5)])
        else:
            utility_score = 0.0
        ...
        if "[No Retrieval]" in pred_text:
            ...
            for order, idx in enumerate(ret_token_appear_indices):
                ...
                for tok, tok_id in ret_tokens.items():
                    prob = pred_log_probs[idx][tok_id] if tok_id in \
                                                          pred_log_probs[
                                                              idx] else -100
                    ret_token_score_dict[order][tok] = np.exp(prob)
                if ret_token_score_dict[order]["[Retrieval]"] + \
                        ret_token_score_dict[order]["[No Retrieval]"] != 0.0:
                    do_retrieve = (ret_token_score_dict[order]["[Retrieval]"] +
                                   ret_token_score_dict[order][
...
def call_model_beam_batch(prompt, model, max_new_tokens=15, ctxs=None,
                          query=None, max_depth=5, rel_tokens=None,
                          grd_tokens=None, ret_tokens=None, threshold=None,
                          beam_width=2, ut_tokens=None, use_seqscore=False,
                          w_rel=1.0, w_sup=1.0, w_use=0.5, ignore_cont=False,
...
def main():
    ...
    ret_tokens, rel_tokens, grd_tokens, ut_tokens = load_special_tokens(
        tokenizer, use_grounding=args.use_grounding,
    ...
    def generate(prompt, ctxs, max_new_tokens):
        processed_prompt = PROMPT_DICT["prompt_no_input"].format_map(
            {"instruction": prompt})
        return call_model_beam_batch(processed_prompt, model=model,
                                     max_new_tokens=max_new_tokens, ctxs=ctxs,
                                     query=prompt,
    ...
    if args.task is not None and args.task == "factscore":
        ...
        for idx, item in enumerate(input_data):
            ...
            new_results.append(
                {"input": item["input"], "output": postprocessed_result,
                 "topic": item["topic"],
                 "cat": item["cat"],
                 "intermediate": intermediate["original_splitted_sentences"][
                     0]})
            if idx % 10 == 0:
        ...
        for instance_idx, item in enumerate(input_data):
            ...
            if "splitted_sentences" not in intermediate:
                ...
                if len(postprocess(final_pred[0])) == 0:
                    intermediate["splitted_sentences"][0], intermediate["ctxs"][
                        0] = intermediate["splitted_sentences"][1], \
                        intermediate["ctxs"][1]
                for idx, (sent, doc) in enumerate(
                        zip(intermediate["splitted_sentences"][0],
                            intermediate["ctxs"][0])):
                    if len(sent) == 0:
                        continue
            ...
            if "original_splitted_sentences" in intermediate:
                item["intermediate"] = \
                    intermediate["original_splitted_sentences"][0]
