...
def parse_args():
    ...
    if args.dataset_name is None and args.train_file is None:
        ...
        if args.train_file is not None:
            ...
            assert extension in ["json",
                                 "jsonl"], "`train_file` should be a json/jsonl file."
    return args


def _tokenize_fn(text: str, tokenizer: transformers.PreTrainedTokenizer,
...
def encode_with_prompt_completion_format(example, tokenizer, max_seq_length,
    ...
    prompt_input, prompt_no_input = PROMPT_DICT["prompt_input"], PROMPT_DICT[
        "prompt_no_input"]
    source_text = prompt_input.format_map(example) if example.get("input",
                                                                  "") != "" else prompt_no_input.format_map(
        example)
    target_text = example['output'] + tokenizer.eos_token
    examples_tokenized = _tokenize_fn(source_text + target_text, tokenizer,
                                      max_seq_length)
...
def encode_with_messages_format(example, tokenizer, max_seq_length):
    ...
    for message_idx, message in enumerate(messages):
        if message["role"] != "assistant":
            ...
            if message_idx < len(messages) - 1 and messages[message_idx + 1][
                "role"] == "assistant":
...
def main():
    ...
    elif isinstance(tokenizer, GPT2Tokenizer) and isinstance(model,
                                                             OPTForCausalLM):
